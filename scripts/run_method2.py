...
from imcpe.methods.cross_stereo_pnp.feature_matching import Method2FeatureMatcher
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Method 2: independent E1-stereo to E2-left PnP per frame."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Sequence name; repeat for several. Default: full split.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="When supplied, also write a separate baseline-fallback variant.",
    )
    ...
    parser.add_argument(
        "--disable-calibration-depth-bounds", action="store_true"
    )
    ...
    parser.add_argument("--fallback-min-inlier-ratio", type=float, default=0.20)
    ...
    config = Method2Config(
        ...
        fallback_min_inlier_ratio=args.fallback_min_inlier_ratio,
    )
    matcher = Method2FeatureMatcher(
        device=args.device, max_num_keypoints=args.max_keypoints
    )
    pure_split_root = args.output_root / "pure" / args.split
    fallback_split_root = args.output_root / "baseline_fallback" / args.split
    ...
    for sequence_dir in tqdm(sequence_dirs, desc="Method 2 sequences"):
        baseline_rows = None
        if args.baseline_root is not None:
            baseline_path = (
                args.baseline_root / args.split / sequence_dir.name / "pose.txt"
            )
            if not baseline_path.is_file():
                raise FileNotFoundError(f"Missing baseline prediction: {baseline_path}")
            baseline_rows = read_pose_txt(baseline_path)
        ...
        sequence_root = pure_split_root / sequence_dir.name
        write_pose_txt(sequence_root / "pose.txt", result.pure_rows)
        if result.fallback_rows is not None:
            write_pose_txt(
                fallback_split_root / sequence_dir.name / "pose.txt",
        ...
        if result.fallback_rows is not None:
            fallback_diagnostics_path = (
                fallback_split_root / sequence_dir.name / "diagnostics.json"
            )
            fallback_diagnostics_path.parent.mkdir(parents=True, exist_ok=True)
            fallback_diagnostics_path.write_text(
                json.dumps(payload, indent=2, default=_json_default) + "\n"
            )
        ...
        for diagnostic in result.diagnostics:
            print(
                ...
                f"E1 stereo matches={diagnostic.e1_stereo_matches} "
                f"E1<->E2 matches={diagnostic.cross_camera_matches} "
                f"shared tracks={diagnostic.shared_e1_left_tracks} "
                f"valid 3D={diagnostic.valid_triangulated_3d} "
                ...
                f"|t|={diagnostic.raw_translation_magnitude_baselines:.3f} "
                f"status={'OK' if diagnostic.pnp_success else diagnostic.failure_reason}"
            )
        print(
            f"SUMMARY: PnP success={100.0 * result.summary['pnp_success_rate']:.1f}% "
            ...
            f"max_t_jump={result.summary['maximum_raw_translation_jump_ratio']:.2f} "
    ...
    print(f"Saved Method 2 pure run: {args.output_root / 'pure'}")
    if args.baseline_root is not None:
        print(
            "Saved separate Method 2 + baseline fallback run: "
            f"{args.output_root / 'baseline_fallback'}"
        )
