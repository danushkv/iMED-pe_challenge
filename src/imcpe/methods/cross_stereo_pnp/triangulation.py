...
def load_e1_stereo_calibration(
    calibration_root: Path, session_id: str
) -> E1StereoCalibration:
    path = calibration_root / f"session_{session_id}.npz"
    if not path.is_file():
        raise FileNotFoundError(f"Missing E1 stereo calibration: {path}")
    with np.load(path, allow_pickle=False) as archive:
        session = str(archive["session_id"].item()).zfill(3)
        if session != str(session_id).zfill(3):
...
def _triangulation_angles(
    ...
    denominator = np.maximum(left_norm * right_norm, np.finfo(np.float64).eps)
    cosine = np.sum(left_rays * right_rays, axis=1) / denominator
    return np.degrees(np.arccos(np.clip(cosine, -1.0, 1.0)))


def triangulate_e1_stereo(
    ...
    if count == 0:
        ...
        return TriangulationResult(
            points_e1_left_3d=np.empty((0, 3), dtype=np.float64),
